...
subject =   {
            'yunnam_simple': [{'sshport':30022,'serviceport':30080,'servicename':'yunnam_simple','network':'172.10.%d.1'}],
...
def main():
    ...
    for team in teams:
        #print team.id,'check dir'
        if not os.path.exists('./run/team_'+str(team.id)):
            #print team.id,'makedirs'
            os.mkdir('./run/team_'+str(team.id))
    ...
    for s in subject:
        ...
        for m in subject[s]:
            md = importlib.import_module('subject.'+m['servicename'])
            img = md.build()
            print img.tags#[0]
            for team in teams:
                
                if os.path.exists('./run/team_'+str(team.id)+'/'+m['servicename']):
                    logger.info('Clearing old floder')
                    cmd = 'rm -rf ./run/team_'+str(team.id)+'/'+m['servicename']
                    os.popen(cmd)
                    logger.info(cmd)

                cmd = 'cp -r ./subject/source/'+s+'/'+m['servicename'] + ' ./run/team_'+str(team.id)
                logger.info(cmd)
                os.popen(cmd)
                ...
                teampass = team.sshpassword
                ip = m['network'] % teamid
                image = img.tags[0]
                #print ip
                mathobj.append(md.o(teamid,teamname, sshport,serviceport,teampass,score,ip,image))
    ...
    threads = [threading.Thread(target=containers_worker) for i in range(thrds)]
